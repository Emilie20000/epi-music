...
const ProductDetailsPage = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const uniqueReviews = Array.from(new Set(data.reviews.map((review) => review.review_id))).map((id) =>
            data.reviews.find((review) => review.review_id === id)
          );
  ...
  const handleColorSelect = (color) => {
    ...
    const sizesForColor = product.models.filter((model) => model.color === color).map((model) => model.size);
  ...
  const promotion = product?.promotions.length > 0 ? product.promotions[0] : null;
    ...
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={product ? `Découvrez ${product.name} sur Epimusic. ${product.description}` : "Découvrez nos produits musicaux sur Epimusic."}
        />
        ...
        <meta property="og:url" content={currentUrl} /> <meta property="og:site_name" content="Epimusic" />
